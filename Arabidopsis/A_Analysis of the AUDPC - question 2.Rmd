...
```
# Arabidopsis Thaliana : AUDPC question 2
...
for (position in 1:3) {
  ...
  plot_t <- plot_t +
    ...
    labs(title = paste("Angle against time for arabidopsis in position ", position, "in normal conditions"), x = "Time", y ="Angle")
  ...
  plot_p <- plot_p +
    scale_color_manual(name = "Angle", values = c("#e78847","#05a894", "#dda0dd", "#2a52be")) +
    labs(title = paste("Angle against time for arabidopsis in position", position, "in changed conditions"), x = "Time", y = "Angle")

  grid <- plot_t + plot_p + plot_layout(ncol = 1, heights = c(1, 1)) +
    plot_annotation(title = paste("Difference in angles between normal and changed conditions for arabidopsis in position", position))
  print(grid)
