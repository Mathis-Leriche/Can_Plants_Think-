...
for (track in 0:2) {
  ...
  if (!all(is.na(track_data_t[, "Angle IN3.Y"]))) {
    ...
    plot_t <- plot_t + 
      ...
      labs(title = paste("Angle against time in tobacco ", track, "in normal conditions"), x = "Time", y ="Angle")
  ...
  plot_p <- plot_p + 
    scale_color_manual(name = "Angle", values = c("#e78847","#05a894", "#dda0dd", "#2a52be")) + 
    labs(title = paste("Angle against time in tobacco ", track, "in changed conditions"), x = "Time", y = "Angle")
  
  grid <- plot_t + plot_p + plot_layout(ncol = 1, heights = c(1, 1)) +
  plot_annotation(title = paste("Difference in angles between normal and changed conditions in tobacco ",track))
  print(grid)
