...
for (track in 0:2) {
  ...
  if (!all(is.na(track_data_t_24h[, "Angle L1.Y"]))) {
    plot_t <- plot_t + geom_line(aes(y = track_data_t_24h[, "Angle L1.Y"], color = "Angle L1.Y"), size = 1) 
  }
  if (!all(is.na(track_data_t_24h[, "Angle L2.Y"]))) {
    plot_t <- plot_t + geom_line(aes(y = track_data_t_24h[, "Angle L2.Y"], color = "Angle L2.Y"), size = 1)
  }
  if (!all(is.na(track_data_t_24h[, "Angle L3.Y"]))) {
    plot_t <- plot_t + geom_line(aes(y = track_data_t_24h[, "Angle L3.Y"], color = "Angle L3.Y"), size = 1)
  }
  if (!all(is.na(track_data_t_24h[, "Angle IN3.Y"]))) {
    plot_t <- plot_t + geom_line(aes(y = track_data_t_24h[, "Angle IN3.Y"], color = "Angle IN3.Y"), size = 1)
  }
    plot_t <- plot_t + 
