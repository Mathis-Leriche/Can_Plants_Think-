...
```
# Nicotinia Tabacuum : AUDPC question 2
...
for (position in 1:3) {
  ...
  pos_t <- list(track = paste0("track_",track), "Angle.L1.Y" = 0, "Angle.L2.Y" = 0, "Angle.L3.Y" = 0, "Angle.IN3.Y" = 0)
  pos_p <- list(track = paste0("track_",track), "Angle.L1.Y" = 0, "Angle.L2.Y" = 0, "Angle.L3.Y" = 0, "Angle.IN3.Y" = 0)
  ...
  if (!all(is.na(position_data_t[, "Angle IN3.Y"]))) {
    ...
    labs(title = paste("Angle against time for Tobacco in position ", position, "in normal conditions"), x = "Time", y ="Angle")
  ...
  if (!all(is.na(position_data_p[, "Angle L3.Y"]))) {
    plot_p <- plot_p + geom_line(aes(y = position_data_p[, "Angle L1.Y"], color = "Angle L1.Y"), size = 1)
  }
  if (!all(is.na(position_data_p[, "Angle IN3.Y"]))) {
    plot_p <- plot_p + geom_line(aes(y = position_data_p[, "Angle L2.Y"], color = "Angle L2.Y"), size = 1)
  }
  plot_p <- plot_p + 
    scale_color_manual(name = "Angle", values = c("#e78847","#05a894", "#dda0dd", "#2a52be")) +
    labs(title = paste("Angle against time for Tobacco in position", position, "in changed conditions"), x = "Time", y = "Angle")

  grid <- plot_t + plot_p + plot_layout(ncol = 1, heights = c(1, 1)) +
    plot_annotation(title = paste("Difference in angles between normal and changed conditions in position", position, "for Tobacco"))
  print(grid)
